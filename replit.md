...
- **God Mode** (`/god-mode`): Super admin-only page hosting `GridOperatorView` (Power Grid Command, Transformer Status, Live Feed, DLQ Health). Reached via dedicated "God Mode" sidebar group above Management. Dashboard no longer renders this inline — operator console has one canonical home.
